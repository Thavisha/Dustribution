...
if __name__=="__main__":
    ...
    input_filename = "user_input.csv" #e.g: "Vela_Cat.csv" #name of .csv file holding catalogue of distances and extinctions and corresponding errors
    ...
    l_lower_train = filled_by_user #e.g: 255.0 #lognitude l in degrees
    l_upper_train = filled_by_user #e.g: 280.0
    n_l_train = filled_by_user #e.g: 30 #number of cells within boundaries - defines resolution of grid  

    b_lower_train = filled_by_user #e.g: -4.0 #latitude b in degrees
    b_upper_train = filled_by_user #e.g: 5.0
    n_b_train = filled_by_user #e.g: 10

    d_min_train = filled_by_user #e.g: 580.0 #Ditance d  in parsecs
    d_max_train = filled_by_user #e.g: 1300.0 
    n_d_train = filled_by_user #e.g: 50
    ...
    l_lower_pred = filled_by_user #e.g: 255.0
    l_upper_pred = filled_by_user #e.g: 280.0
    n_l_pred = filled_by_user #e.g: 150
   
    b_lower_pred = filled_by_user #e.g: -4.0
    b_upper_pred = filled_by_user #e.g: 5.0
    n_b_pred = filled_by_user #e.g: 50
    
    d_min_pred = filled_by_user #e.g: 580.0
    d_max_pred = filled_by_user #e.g: 1300.0 
    n_d_pred = filled_by_user #e.g: 200
    ...
    scale_length_x = filled_by_user #e.g: 10.0 #Scale length - parsec units - #Approximated from literature given size of structure in region of interest dependent on how close/far we are from the source allowing us to recover the smaller/larger structure. If the region of interest is further away from us we will likely only be able to recover the large scale structure so we need a larger scale length to reflect the size of the large scale structure. But if we are close by we maybe sensitive to the small scale structure as well so we need a smaller scale lenght to reflect the smaller stucture sizes.  
    scale_length_y = filled_by_user #e.g: 10.0
    scale_length_z = filled_by_user #e.g: 10.0 
    mean_ext_dens = filled_by_user #e.g: -3.333 #-3.333 #Mean density - log10(Mags per pc) units - Approximate mean slope of the extinction (units=Mag per pc) - determined from literature
    exp_scalefac = filled_by_user #e.g: -1.215 #-1.215 #ln(Scale factor) kernel - approximate size of offset expected from the mean -  determined from literature and fit trials


    #Pyro/ADAMW ELBO optimisation paramters
    learning_rate = filled_by_user #e.g: 0.01 #Scales the size of steps taken when optimising the free paramters with each itteration - Bigger lr means fewer itterations and less likely to get stuck in a local minimum, but if the lr is too small we'll need too many steps to reach a solution
    learning_eps = filled_by_user #e.g: 1e-8 #Epsilon value for ADAMW - To stabalise the steps taken and avoid large negative ELBO jumps
    num_iter =  filled_by_user #e.g: 2500 #Number of gradient descent steps - needs to run until the ELBO flattens out and the model has converged - we use a <1% change in ELBO over the last 10 itterations to stop
    num_particles =  filled_by_user #e.g: 32 #Number of sample graident calculations at each gradient descent step - power of two values work best
    num_inducing =  filled_by_user #e.g: 1000 #Number of inducing points to use - number of positions in the conditon grid used to optimise the density distribution. 
    min_iter =  filled_by_user #2000 #Minimum number of itterations to run before imposing stopping criterion
    stop_prcnt =  filled_by_user #e.g: 0.001 #Percentage (fractional) change of the ELBO below which the training stops. We use 1%(0.01) 
    stop_iter = filled_by_user #e.g: 10 #Number of iterations to look back at to calculate the average ELBO change to impose the stopping criterion
    snapshot_iter = filled_by_user #e.g: 5 #At each snapshot_iter take a snapshot of the full GP so that it can be loaded and training can begin from their rather than fully restarting if needed
   
    #Prediction set up
    pred_chunk_size = filled_by_user #e.g: 5000 #Number of prediction grid chunks to be used at one given time to predict on till the full predict grid is filled
    pred_sample_size = filled_by_user #e.g: 1000 #Number of density/extinction samples to draw from final gp to obtain the percentiles for prediction - We obtain the 16th, 50th, 84th percentiles


    #Rerun statuses if we want to recreate grids or retrain GP from the stars
    recalc_grid_train = filled_by_user #True/False #If we do/do not want to recalculate the training grid defined based on the lbd coordinates and n resolution above - must be recalculated if the coordinates or resolutions deifned above change
    recalc_grid_pred = filled_by_user #True/False #If we do/do not want to recalculate the predicting grid defined based on the lbd coordinates and n resolution above - must be recalculated if the coordinates or resolutions deifned above change
    recheck_sourcebounds = filled_by_user #True/False #If we do/do not want to reheck source positions in source data frame to make sure their within our input coorindates and removing any which are not. Also re-precomputes some source indeces for integration optimisation
    retrain_gp = filled_by_user #True/False  #If we do/do not want to rerun the full GP training based parameters given above - must be recalculated if any of the parameters above change except the prediciting grid parameters
    repredict_gp = filled_by_user #True/False  #If we do/do not want to rerun the GP prediction based parameters given above - must be recalculated if any of the parameters above change
 
    
    #Run Algorithm on GPUs or CPUs
    train_gpu = filled_by_user #Set True for GPU run or set False for CPU run for the GP training
    pred_gpu = filled_by_user #Set True for GPU run or set False for CPU run for the GP predicting. #We also need to set this to False if we're only plotting in a machine with No GPU
    plot_gpu = filled_by_user  #Set True for plotting in the GPU or set False for CPU for plotting following GPU training especially if plotting is done outside a machine with a GPU
